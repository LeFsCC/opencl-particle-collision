...
void Spheres::reset() {
	...
	set_array(POS, cpu_pos, 0, num_particles);
	set_array(VEL, cpu_vel, 0, num_particles);
}
...
void Spheres::update(float deltaTime) {
	...
	cpu_pos = get_array(POS);
}

float* Spheres::get_array(attr_type array) {
	float* cpudata = 0;
	cl_mem gpudata = 0;
	switch (array) {
		case POS:
...
void Spheres::set_array(attr_type array, const float* data, int start, int count) {
	switch (array) {
	case POS:
